...
Four things ended up as isolated agents (`risk-classifier`, `story-converter`, `validator`, `implementer`); one thing that looked like it should be a fifth deliberately isn't (`decision-recorder`). The difference: `validator` benefits from fresh eyes — no memory of how the code was built is the point. `decision-recorder` is the opposite — its whole job is capturing reasoning that just happened in the calling session, so isolating it means re-explaining that reasoning explicitly on every call, at the highest invocation frequency in the system (3+ times per story plus every amendment), for a worse result, not a better one. `implementer` went through a conditional-isolation phase (forked only for `parallel_group` tasks, inline otherwise) and was reverted to always-isolated — the conditional version traded a real, immediate cost (inconsistent model quality on solo tasks) for a theoretical, never-measured savings. Don't isolate by default; check whether the specific component's job benefits from a blank slate or is actively hurt by one.
