...
function rowToStory(row) {
    return {
        ...
        context_mode: String(row.context_mode),
        depends_on: row.depends_on ? JSON.parse(String(row.depends_on)) : undefined,
    };
}
export async function getBacklog(client) {
    const metaResult = await client.execute("SELECT project, program_spec_ref, architecture_mode FROM backlog_meta WHERE id = 1");
    const storiesResult = await client.execute("SELECT id, title, ticket_id, acceptance_criteria, context_mode, depends_on FROM stories ORDER BY id");
...
export async function getStory(client, storyId) {
...
export async function setBacklogMeta(client, meta) {
...
export async function addStory(client, story) {
...
function formatDecisionId(seq) {
    return `DEC-${String(seq).padStart(4, "0")}`;
}
export async function appendDecision(client, input) {
...
export async function listDecisions(client, opts = {}) {
    const conditions = [];
    const args = [];
    if (opts.story_id) {
